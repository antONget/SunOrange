...
async def start(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    await message.answer(text=f'Здравствуйте! 🍊 Мы рады помочь вам в подборе идеального тура! Прежде чем мы подключим'
                              f' нашего эксперта, не могли бы вы ответить на несколько простых вопросов? Это поможет'
                              f' нам лучше понять ваши предпочтения и пожелания.\n\n'
                              f'Давайте познакомимся! Как вас зовут?')
